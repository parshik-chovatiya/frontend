...
const DoseCard: React.FC<DoseCardProps> = ({ reminder, time }) => {
            ...
            <div className="flex flex-1 items-center justify-between min-w-0">
                <div className="flex-1">
                    <p className="text-sm text-muted-foreground mb-1">
                        Take {reminder.amount} {reminder.medicine_type}
                    </p>
                    <h3 className="text-base font-bold text-foreground">{reminder.medicine_name}</h3>
                </div>
                <div className="shrink-0 ml-4">
                    <p className="text-sm text-muted-foreground flex items-center gap-1">
                        <ClockIcon />
...
const UpcomingDosesSection: React.FC<{ dashboardData: DashboardData | null }> = ({ dashboardData }) => {
    if (!dashboardData || dashboardData.doses.length === 0) {
        return (
            <div className="rounded-lg bg-white p-6 shadow-lg flex items-center justify-center">
                <p className="text-muted-foreground">No doses scheduled for this date</p>
            </div>
        );
    }
    ...
    return (
        <div className="rounded-lg bg-white p-6 overflow-hidden flex flex-col shadow-lg">
            <h2 className="text-2xl font-bold mb-2">Upcoming Doses</h2>
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar]:h-4 [&::-webkit-scrollbar-thumb]:bg-gray-400 [&::-webkit-scrollbar-thumb]:rounded-full">
                {dashboardData.doses.map((timeSlot) =>
                    timeSlot.reminders.map((reminder) => (
                        <DoseCard
                            key={`${reminder.reminder_id}-${reminder.dose_number}`}
                            reminder={reminder}
                            time={timeSlot.time}
                        />
                    ))
...
export default function Dashboard() {
    ...
    const fetchDashboardData = async (date: string) => {
        try {
            const response = await axiosInstance.get(`/reminders/dashboard/?date=${date}`);
    ...
    return (
        <>
            ...
            <div className="h-[calc(100vh-10rem)]">
                <main className="pb-4">
                    <DateStrip onDateSelect={setSelectedDate} />
                </main>
                <div className="grid h-84 grid-cols-2 gap-4 px-4">
                    <UpcomingDosesSection dashboardData={dashboardData} />

                    <div className="grid grid-cols-2 grid-rows-2 gap-4">
                        <div className="rounded-lg bg-white flex items-center justify-center shadow-lg">
                            <div className="text-center">
                                <p className="text-3xl font-bold text-primary">{dashboardData?.total_doses || 0}</p>
                                <p className="text-sm text-muted-foreground">Total Doses</p>
                            </div>
                        </div>
                        <div className="rounded-lg bg-white flex items-center justify-center shadow-lg">
                            <div className="text-center">
                                <p className="text-3xl font-bold text-primary">{dashboardData?.total_reminders || 0}</p>
                                <p className="text-sm text-muted-foreground">Active Reminders</p>
                            </div>
                        </div>
                        <div className="rounded-lg bg-white flex items-center justify-center shadow-lg">
                            <div className="text-center">
                                <p className="text-lg font-bold">{dashboardData?.day_name || ''}</p>
                        ...
                        <div className="relative rounded-2xl bg-white shadow-lg overflow-hidden p-6 flex flex-col justify-between">
                            ...
                            />
                            <div className="relative z-10">
                                <h3 className="text-2xl text-blue-600 mb-2" style={{ fontFamily: "Caveat, cursive" }}>
                                    Health Tip of the Day
                                </h3>
                                <Image src="/underline.svg" alt="" width={140} height={10} className="-mt-2 mb-2" />
                                <p className="text-sm text-gray-800 leading-relaxed max-w-[90%] font-semibold">
                                    Don't skip doses even if you feel better—finish the course.
                                </p>
                            </div>
